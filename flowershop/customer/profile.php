...
// Fetch current data with error checking
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['upload_photo'])) {
    if (isset($_FILES['profile_image']) && $_FILES['profile_image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (empty($uploadErrors)) {
            ...
            if (!empty($user['profile_image']) && file_exists($_SERVER['DOCUMENT_ROOT'] . '/flowershop/' . $user['profile_image'])) {
                unlink($_SERVER['DOCUMENT_ROOT'] . '/flowershop/' . $user['profile_image']);
            }
            ...
            if (move_uploaded_file($fileTmp, $uploadPath)) {
                ...
                $uploadErrors[] = 'Failed to upload image. Please try again.';
            }
        }
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['change_password'])) {
    ...
    if (!password_verify($current, $user['password']))   $pwErrors[] = 'Current password is incorrect.';
    elseif (strlen($newPass) < 6)                        $pwErrors[] = 'New password must be at least 6 characters.';
    elseif ($newPass !== $confirm)                       $pwErrors[] = 'New passwords do not match.';
  ...
  <div class="card">
    ...
    <div class="card-body">
      ...
      <div style="text-align:center;margin-bottom:24px;">
        <?php 
        $profileImage = !empty($user['profile_image']) ? APP_URL . '/' . $user['profile_image'] : null;
        ?>
        <div style="position:relative;display:inline-block;">
          ...
          <?php else: ?>
            <div style="width:100px;height:100px;border-radius:50%;background:var(--accent);color:#fff;
                 display:flex;align-items:center;justify-content:center;font-size:2.5rem;font-weight:700;margin:0 auto 10px;border:3px solid var(--accent);">
  ...
  <div>
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        $result = $conn->query("SELECT COUNT(*) as v FROM orders WHERE customer_id=$uid");
        if ($result) {
            $totalOrders = $result->fetch_assoc()['v'];
            $result->free();
        }
        
        $result = $conn->query("SELECT COALESCE(SUM(total_amount),0) as v FROM orders WHERE customer_id=$uid AND status!='cancelled'");
        if ($result) {
            $totalSpent = $result->fetch_assoc()['v'];
            $result->free();
        }
        
        $result = $conn->query("SELECT COUNT(*) as v FROM enquiries WHERE customer_id=$uid");
        if ($result) {
            $enquiries = $result->fetch_assoc()['v'];
            $result->free();
        }
        ...
        <div style="display:grid;gap:12px;">
          <div style="display:flex;justify-content:space-between;align-items:center;padding:10px;background:var(--sand);border-radius:8px;">
            <span class="text-muted"><i class="fa fa-bag-shopping"></i> Total Orders</span>
            <strong><?= $totalOrders ?></strong>
          </div>
          <div style="display:flex;justify-content:space-between;align-items:center;padding:10px;background:var(--sand);border-radius:8px;">
            <span class="text-muted"><i class="fa fa-coins"></i> Total Spent</span>
            <strong style="color:var(--accent);"><?= formatCurrency($totalSpent) ?></strong>
          </div>
          <div style="display:flex;justify-content:space-between;align-items:center;padding:10px;background:var(--sand);border-radius:8px;">
            <span class="text-muted"><i class="fa fa-comment-dots"></i> Enquiries</span>
            <strong><?= $enquiries ?></strong>
          </div>
          <div style="display:flex;justify-content:space-between;align-items:center;padding:10px;background:var(--sand);border-radius:8px;">
            <span class="text-muted"><i class="fa fa-calendar"></i> Member Since</span>
